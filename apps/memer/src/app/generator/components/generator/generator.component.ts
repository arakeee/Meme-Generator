...
export class GeneratorComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.generatorForm.get('memeSearch')?.valueChanges
      .pipe(
        debounceTime(1000),
  ...
  generateMeme() {
    ...
    this.genService.createMeme({
      ...
      catchError((err) => {
        ...
        switch(err.status) {
          case 429:
            this.generationError = 'Too many requests. You can only generate 3 memes in 5 minutes. ';
            break;
